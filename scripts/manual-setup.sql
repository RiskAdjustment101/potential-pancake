-- MANUAL SETUP: Copy this entire SQL and run in Supabase SQL Editor
-- Go to: https://supabase.com/dashboard/project/lodmtemrzvmiihfoidrt/sql/new
...
CREATE TABLE IF NOT EXISTS parent_comms (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_season_weeks_season_plan_id ON season_weeks(season_plan_id);
CREATE INDEX IF NOT EXISTS idx_weekly_agendas_user_id ON weekly_agendas(user_id);
CREATE INDEX IF NOT EXISTS idx_weekly_agendas_season_week_id ON weekly_agendas(season_week_id);
CREATE INDEX IF NOT EXISTS idx_parent_comms_user_id ON parent_comms(user_id);
CREATE INDEX IF NOT EXISTS idx_parent_comms_season_plan_id ON parent_comms(season_plan_id);
...
DROP TRIGGER IF EXISTS update_season_weeks_updated_at ON season_weeks;
CREATE TRIGGER update_season_weeks_updated_at BEFORE UPDATE ON season_weeks 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_weekly_agendas_updated_at ON weekly_agendas;
CREATE TRIGGER update_weekly_agendas_updated_at BEFORE UPDATE ON weekly_agendas 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_parent_comms_updated_at ON parent_comms;
...
SELECT 'Tables created successfully!' as status,
       (SELECT COUNT(*) FROM information_schema.tables 
        ...
        AND table_name IN ('season_plans', 'season_weeks', 'weekly_agendas', 'parent_comms')) as table_count;